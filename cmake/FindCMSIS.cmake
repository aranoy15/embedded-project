...
function(cmsis_flash_offset OFFSET FLASH SIZE OUT_FLASH OUT_SIZE)
    set(LOCAL_FLASH ${FLASH})
    set(LOCAL_SIZE ${SIZE})
    set(KB_OFFSET ${OFFSET})

    string(REGEX REPLACE "[^0-9]+" "" LOCAL_SIZE ${LOCAL_SIZE})

    math(EXPR KB_OFFSET "${OFFSET}/0x400" OUTPUT_FORMAT DECIMAL)

    math(EXPR LOCAL_FLASH "${FLASH}+(${OFFSET})" OUTPUT_FORMAT HEXADECIMAL)
    math(EXPR LOCAL_SIZE "${LOCAL_SIZE}-${KB_OFFSET}" OUTPUT_FORMAT DECIMAL)

    set(LOCAL_SIZE "${LOCAL_SIZE}K")

    set(${OUT_FLASH} ${LOCAL_FLASH} PARENT_SCOPE)
    set(${OUT_SIZE} ${LOCAL_SIZE} PARENT_SCOPE)
endfunction()
...
function(cmsis_generate_default_linker_script FAMILY DEVICE CORE)
    ...
    if (NOT FLASH_OFFSET)
        set(FLASH_OFFSET 0x00)
    endif()

    set(RESULT_FLASH ${FLASH_ORIGIN})
    set(RESULT_SIZE ${FLASH_SIZE})

    cmsis_flash_offset(${FLASH_OFFSET} ${FLASH_ORIGIN} ${FLASH_SIZE} RESULT_FLASH RESULT_SIZE)

    add_custom_command(OUTPUT "${OUTPUT_LD_FILE}"
        COMMAND ${CMAKE_COMMAND} 
            -DFLASH_ORIGIN="${RESULT_FLASH}" 
            -DRAM_ORIGIN="${RAM_ORIGIN}" 
            -DCCRAM_ORIGIN="${CCRAM_ORIGIN}" 
            -DFLASH_SIZE="${RESULT_SIZE}" 
            -DRAM_SIZE="${RAM_SIZE}" 
